...
class MapBlueprintTest extends TestCase
{
    ...
    public function it_execute_a_map_statement_from_blueprint()
    {
        ...
        $connection = Mockery::mock(\Nuwber\Oponka\Connection::class);
        $connection->shouldReceive('mapStatement')->withArgs([$assertion])->once()->andReturn(['ok']);
        $grammar = Mockery::mock(\Nuwber\Oponka\Map\Grammar::class);
        $grammar->shouldReceive('compileCreate')->once()->andReturn(['foo' => 'bar']);

        $blueprint = new \Nuwber\Oponka\Map\Blueprint('posts');
        $blueprint->create();
