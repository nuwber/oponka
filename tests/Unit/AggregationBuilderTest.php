...
class AggregationBuilderTest extends TestCase
{
    ...
    {
        $builder = $this->getBuilder();
        $builder->geoDistance('foo', 'bar', 'baz', [['from' => 0.1], ['to' => 0.3]]);
        $this->assertEquals([
            'aggregations' => [
                'foo' => [
                    'geo_distance' => [
    ...
    {
        $builder = $this->getBuilder();
        $builder->ipv4Range('foo', 'bar', [['from' => '127.0.0.1', 'to' => '127.0.0.224']]);
        $this->assertEquals([
            'aggregations' => [
                'foo' => [
                    'ip_range' => [
                        'field'  => 'bar',
                        'ranges' => [['from' => '127.0.0.1', 'to' => '127.0.0.224']],
                    ],
                ],
            ],
        ], $builder->toDSL());
    ...
    {
        $builder = $this->getBuilder();
        $builder->percentile('foo', 'bar', [50, 75, 95]);
        $this->assertEquals(
            ['aggregations' => ['foo' => ['percentiles' => ['field' => 'bar', 'percents' => [50, 75, 95]]]]],
            $builder->toDSL()
        );
    }
    ...
    {
        $query = new \OpenSearchDSL\Search();

        return new \Nuwber\Oponka\DSL\AggregationBuilder($query);
    }
}
